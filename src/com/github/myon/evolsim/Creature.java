...
public class Creature implements Locateable<Creature>, WorkItem {
	...
	public void run() {
		...
		final Creature that = this.world.get().getCollisionSpace().checkPoint(this.getLoaction().x(),
				this.getLoaction().y(), this.location, null);
		if (that != null) {
			this.getLoaction().xy(Math.min(0.01 / (this.getLoaction().x() - that.getLoaction().x()), this.getRadius()),
	...
	public void move(final Neuron neuron) {
		...
		final double r = neuron.position().length()
				* Math.cos(neuron.position().orientation() - neuron.position().angle());
		...
		// System.out.println(this.name + " moved a:"+ (r/(2*Math.PI)) + " x:" +
		// fx + " y:" + fy);
		this.orientation += r;
	...
	public void predate(final Neuron neuron) {
		final double x = this.location.x()
				+ Math.cos(this.orientation + neuron.position().orientation()) * neuron.position().x();
		final double y = this.location.y()
				+ Math.sin(this.orientation + neuron.position().orientation()) * neuron.position().y();
